...
void	print_ids(t_bus *buses)
{
	...
	{
		printf("%lu : %lu : %d\n", buses[i].value, buses[i].original_id, buses[i].timestamp);
		// printf("%lu : %d : %lu\n", buses[i].value, buses[i].timestamp, buses[i].value - buses[0].value);
	}
...
size_t	get_timestamp(t_bus *buses)
{
	...
	while (1)
	{
		...
		while (i < BUS_COUNT)
		{
			if ((buses[0].value + buses[i].timestamp) % buses[i].original_id != 0)
				break ;
			i++;
		}
		if (i == BUS_COUNT)
